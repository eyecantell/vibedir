...
class VibedirMessage:
    ...
    def to_markdown_header(self, model: str = "grok-4") -> str:
        ...
        now = self.timestamp or datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:23]
        if self.role == "user":
...
class VibedirPromptParser:
    ...
    def write_pending(self, pending_text: str):
        ...
        with open(self.file_path, "w", encoding="utf-8") as f:
            f.write(f"# vibedir session - {datetime.now().isoformat(timespec='milliseconds')}\n\n")
            for msg in history:
